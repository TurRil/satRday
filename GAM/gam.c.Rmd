...
# two zones - left or right handed
```
...
# x0:az contains info on trajectory
...
# info on tuning parameters (k variable)
...
Gradient range [3.603818e-09,3.603818e-09]
...
s(pz) 99.00  7.16    1.01    0.78

# low p-value indicates under-smoothing
...
```{r}
fit1 <- bam(called ~ s(px, pz, k = 100),   # only included vertical direction
           family = "binomial",
           data = strikes)

fit2 <- bam(called ~ s(px, pz, k = 100) + # add velocity in vertical
               s(vy0, vz0, k = 100),
           family = "binomial",
           data = strikes)
```
