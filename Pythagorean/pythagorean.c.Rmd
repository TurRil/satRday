...
[https://www.basketball-reference.com/leagues/NBA_2017_games-october.html](https://www.basketball-reference.com/leagues/NBA_2017_games-october.html")
...
- Let's use all but the last month of the regular season to fit the Pythagorean model
 (use last month as sort off validation)
...
# transform
#   Team_A vs Team_B points
#   Team_A vs Team_C points
# to
#   Team_A Pts_Won Pts_Lost
#   Team_B Pts_Won Pts_Lost
#   Team_C Pts_Won Pts_Lost
...
fit <- glm(Odds ~ -1 + log(PtsWon / PtsLost),
           data = train_data %>% filter(Match > 20)) #ignore the first 20 mathces - burn in period
```

```
> fit

Call:  glm(formula = Odds ~ -1 + log(PtsWon/PtsLost), data = train_data %>% 
    filter(Match > 20))
...
Degrees of Freedom: 1188 Total (i.e. Null);  1187 Residual
Null Deviance:	    585 
Residual Deviance: 64.18 	AIC: -91.59
...
Call:
glm(formula = Odds ~ -1 + log(PtsWon/PtsLost), data = train_data %>% 
    filter(Match > 20))

Deviance Residuals: 
     Min        1Q    Median        3Q       Max  
-0.71371  -0.13218   0.00892   0.17333   0.79163  

Coefficients:
                    Estimate Std. Error t value Pr(>|t|)    
log(PtsWon/PtsLost)  14.3866     0.1466   98.15   <2e-16 ***

### alpha value of 14 ###
### baseball is 2 is lower scoring game

---
Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1

(Dispersion parameter for gaussian family taken to be 0.05406868)

    Null deviance: 585.04  on 1188  degrees of freedom
Residual deviance:  64.18  on 1187  degrees of freedom
AIC: -91.593
...
alpha value of 14
baseball alpha is 2 = lower scoring game
basketball the score is higher


Now: compare to out of sample data - compare to last month
