...
1. Limit the dataset to 2017 men's matches, because we want to use `RallyCount`
...
"Free Point" on first or 2nd serve
...
1. Empirical mean and standard deviation for each variable,
   plus standard error of the mean:
     ...
     0.3832683      0.0078688      0.0002488      0.0014031 
     
## free point on first point chance:  38%

2. Quantiles for each variable:

  2.5%    25%    50%    75%  97.5% 
0.3684 0.3778 0.3833 0.3887 0.3994 

```

/u represents average free point: get log odds of it > apply exp(x)
...
# have an alpha/theta for each match - take median of a player
...
# introduce new.total (test) to include test
...
jags <- rjags::jags.model(textConnection(modelString),
                   data = list('J' =  max(trainData$j),
                   			   ...
                   			   'n' = trainData$n,
                   			   'N' = nrow(trainData),
                   			   'test.j' = testData$j,   ## test data set
                   			   'test.k' = testData$k,
                   			   'test.n' = testData$n,
                   			   'M' = nrow(testData)
                               ),
